...
from mcstas_nxs_inst import NX2COMP_MAP, AffineRotate, NXoff
...
def nx2mcstas(nxfile):
    #instname = os.path.basename(nxfile).replace('.' + nxfile.split('.')[-1], '')
    #print(nxinstr)
    #print(nxinstr.tree)
    instname = os.path.basename(nxfile).replace('.', '_')
    nx_inst = get_nx_component(nxload(nxfile), nxtype=nexus.NXinstrument)
    mc_inst = NXinst2McStas(instname, nx_inst)
    mc_inst.show_components()
