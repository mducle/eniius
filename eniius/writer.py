...
def new_wd(self, data):
    nxpath = f'{self.nxpath}/{data.nxname}'
    if data._target is None and data._uncopied_data is None and data._memfile is None \
            ...
            and data.is_string() and isinstance(data._value, str):
        data._shape = (1,)
...
class Writer:
    ...
    def to_nxspe(self, outfile, ei=25):
        ...
        with nxopen(outfile, 'w') as root:
            ...
            if det_file:
                ...
                for fnm, idn in zip(['detectors', 'monitors'], [2, 1]):
                    ...
                    fd['user_table_titles'] = NXfield(np.array(titles, dtype='S'), shape=(1,))
                    ...
                    root['mantid_workspace_1/instrument/physical_' + fnm] = NXdetector(**fd)
